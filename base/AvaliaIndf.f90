...
!	             Variáveis Auxiliares
	...
	REAL :: LLJ,LLV, RPILAR, RO
	REAL :: LP, SP 

	REAL :: LLJC     ! Vão laje corrigido (m)
...
!
	INTEGER  QDP  ! Qdes de viga, laje e pilar
	REAL :: QDV,QDL
	
!            VARIÁVEL DOS CUSTOS	
		
	REAL :: DOP, CDOP
	REAL :: CTV, CTL, CTP, CTT
	REAL :: CTMV, CTML, CTMP, CTMT
	REAL :: CUSTOEST, CUSTONEO			   
!
!	               Variáveis da Laje
!
...
!
	...
	REAL :: CUSTOCIS,vtc,CUSTOPE,CUSTOLIG
	real pentotal            ! Somatório Penalidades
	REAL :: LPL,NLP,QDPL,PTAL,LPV,NVP,QDPV,PTAV


!IF (K.EQ.1) THEN    !  TESTE PARA EVITAR VÁRIAS LEITURAS
...
!
	...
	DL=SUBPOP(k,1)	                                     ! Já é o próprio valor

	PM=2*SUBPOP(k,2)+1*SUBPOP(k,3)+1   ! Auxiliar
	CML=2*SUBPOP(k,4)+1*SUBPOP(k,5)+1   ! Auxiliar

	VL=16*SUBPOP(k,6)+8*SUBPOP(k,7)+4*SUBPOP(k,8)+2*SUBPOP(k,9) &
	+1*SUBPOP(k,10)+1	 ! Auxiliar

	ANPT=2*SUBPOP(k,11)+1*SUBPOP(k,12)+1         ! Auxiliar Lista NPT

	ABP=2*SUBPOP(k,13)+1*SUBPOP(k,14)+1                      ! Auxiliar p a lista BP

	ANA=8*SUBPOP(k,15)+4*SUBPOP(k,16)+2*SUBPOP(k,17)+1*SUBPOP(k,18)+1	 ! Auxiliar 
	
	ANB=4*SUBPOP(k,19)+2*SUBPOP(k,20)+1*SUBPOP(k,21)+1				 ! Auxiliar

	NX=0

	DO i=1,NXMAX

	NX=NX+SUBPOP(k,(21+i))*(2**(NXMAX-i))
	...
	NY=NY+SUBPOP(k,(21+NXMAX+i))*(2**(NYMAX-i))
	...
	VV=VV+SUBPOP(k,(21+NXMAX+NYMAX+i))*(2**(NVV-i))
...
!
	...
	ZLA=DDL-0.4*XL
	ZLB=DDL-0.02
	MRESL=0.0291*fckml(cml)*ZLB+0.58285*(XL-0.05)*FCKPM(PM)*ZLA
...
!	IF (TTS.LE.(2*LTJ)) THEN
	IF (TTS.LE.((07*fckpm(pm))**0.5)) THEN
	                GL(6)=0
	                ELSE
...
!		ELU e Beta X
   ...
   ENDIF
	...
	BXL=XL/DDL						 ! Beta X Laje
	                                 ! O limite do domínio 3 =0,258, MAS:
									 ! permite-se q a peça esteja no domínio 2


	IF (BXL.GT.0) THEN 				   
	                  GL(13)=0
	                  ELSE 
					  GL(13)=ABS(BXL)    ! Evitar q o beta X seja menor q "0"
	ENDIF
	...
	IF ((ft+cft).LE.(1.05*LLJ/250))      THEN
	                                GL(16)=0
									ELSE
...
!		DESCOMPRESSAO
	...
	IF (DESCOMPV.LE.0.05)   THEN  ! Na realidade este valor é zero
								  ! mas adotei 0.05 como uma margem e uma referência
	                     GV(9)=0				   
	                     ELSE 
...
!		ELU e Beta X
	...
	IF (BXV.GT.0)   THEN 
	                    GV(12)=0				! Beta X Viga
	                    ELSE 
						GV(12)=ABS(BXV)			! Evitar q o BX seja "0"
	ENDIF
...
!	   Geometria
    ...
    IF (BV(VV).LE.BMAX) THEN 
	                    ...
	                    ELSE 
						GV(17)=((100*BV(VV))/BMAX)-1  !TESTE
	ENDIF
	    
	IF (LLV.GE.(DMINX*(DL)+DMINY*(1-DL))) THEN 
	                                      GV(18)=0
		                                  ELSE 
...
!
	QDV=NX*(NY+1)*DL+NY*(NX+1)*(1-DL) ! Qde de vigas POR PAVIMENTO
	  	
	QDL=(REAL(NX)*LLV/1.2)*NY*DL+(REAL(NY)*LLV/1.2)*NX*(1-DL) ! Qde de lajes
															  ! POR PAVIMENTO
...
!				   e o da viga da largura do pilar
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	VPMP=SP*3.5*1.15           				 ! Volume Pilar - APENAS O LANCE
...
!PTAL=((QDPL*LPL)-(QDL*LLJC))*APL(VL)*7857 ! PERDA DO AÇO DE PROTENSAO EM KG(laje)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	CUSTOAD=(VADV*7760*QDV*CAD+VPMP*RO*QDP*CAD)*NUMPAV	 ! ARM PASSIVA VIGA E PILAR
	...
	CUSTONEO=QDP*2*NUMPAV*22.5    ! CUSTO NEOPRENE R$ 22,50
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	CUSTOTAL=CUSTOFAB+CUSTOCONCML+CTT+CTMT+CUSTONEO+CUSTOLIG

	! 1.33 * PARA CONSIDERAR OS CUSTOS FIXOS

	Custoest=Custotal-CUSTOCONCML   ! Custo apenas ESTRUTURA PM

	CUSTOTAL=CUSTOTAL/NUMPAV
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!																!
!                Resultados que serão Impressos					!
!																!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	vtc=vpmv*qdv+vpml*qdl+vpmp*qdp+vml
	
	WRITE (6, '(/,/,1X, A,I3, A)') "--- Variáveis Projeto individuo ", k, " ---"
		
		WRITE (6, '(1X,A,I3,A)') "NumGen= ", NUMGEN
		WRITE (6, '(1X,A,I3,A)') "NX= ", NX
    	WRITE (6, '(1X,A,I3,A)') "NY= ", NY
		WRITE (6, '(1X,A,I3,A)') "DL= ", DL
		WRITE (6, '(1X,A,I3,A)') "FCKCML= ", FCKML(CML)
		WRITE (6, '(1X,A,I3,A)') "FCKCPM= ", FCKPM(PM)
		WRITE (6, '(1X,A,F6.2,A)') "HL= ", HL(VL)
		WRITE (6, '(1X,A,F6.2,A)') "LLJ= ", LLJ
		WRITE (6, '(1X,A,F6.2,A)') "HV= ", HV(VV)
		WRITE (6, '(1X,A,F6.2,A') "BV= ", BV(VV)
		WRITE (6, '(1X,A,F6.2,A)') "LLV= ", LLV
		WRITE (6, '(1X,A,I3,A)') "NA= ", NA(ANA)
		WRITE (6, '(1X,A,I3,A)') "NB= ", NB(ANB)
		WRITE (6, '(1X,A,I3,A)') "NPT= ", NPT(ANPT)
		
		
		WRITE (6, '(1X,A,E8.2,A)') "Custo Concreto= ", custoconc
		WRITE (6, '(1X,A,E8.2,A)') "Volume Concreto= ", vtc
		WRITE (6, '(1X,A,E8.2,A)') "Custo Protensão= ", custoprot
		WRITE (6, '(1X,A,E8.2,A)') "Custo Aço Passivo= ", custoad
		WRITE (6, '(1X,A,E8.2,A)') "Custo Transporte= ", CTT
		WRITE (6, '(1X,A,E8.2,A)') "Custo Montagem= ", CTMT
			WRITE (6, '(1X,A,E8.2,A)') "teste= ", CTMv


		WRITE (6, '(1X,A,E8.2,A)') "Custo Desp Operacional= ", CDOP

		WRITE (6, '(1X,A,E8.2,A)') "Custo %Fabricacao= ", (CUSTOFAB/CUSTOEST)*100
		                                                   
		WRITE (6, '(1X,A,E8.2,A)') "Custo %Transporte= ", (ctt/custoest)*100
		WRITE (6, '(1X,A,E8.2,A)') "Custo %Montagem= ", (ctmt/custoest)*100




		
		WRITE (6, '(1X,A,E10.4,A)') "Custo Total= ", custotal
		WRITE (6, '(1X,A,E8.2,A)') "Pentotal= ", pentotal
		WRITE (6, '(1X,A,E10.4,A)') "Apt= ", F
		WRITE (6, '(1X,A,E10.4,A)') "Custo Estrutura/m2= ", (Custoest*1.33)/(lx*ly*NUMPAV)
		
		!  1.33  INCLUIR O LUCRO E IMPOSTOS

	
	WRITE (6, '(/,/,1X, A,I3,A)') "--- DADOS DA LAJE INDIVÍDUO ", k, " ---"
	
		WRITE (6, '(1X,A,I3,A)') "VL= ", VL
!		WRITE (6, '(1X,A,E8.2,A)') "XL= ", XL
!		WRITE (6, '(1X,A,E8.2,A)') "Mres= ", mresl
		WRITE (6, '(1X,A,E8.2,A)') "cf-Total (Laje)= ", cft
!		WRITE (6, '(1X,A,E8.2,A)') "I isolada= ", II(vl)
!		WRITE (6, '(1X,A,E8.2,A)') "I composta= ", IC
!		WRITE (6, '(1X,A,E8.2,A)') "ygc= ", ygc
!		WRITE (6, '(1X,A,E8.2,A)') "AC= ", AC
!		WRITE (6, '(1X,A,E8.2,A)') "m= ", m
		WRITE (6, '(1X,A,E8.2,A)') "flecha (Laje)= ", fi
		WRITE (6, '(1X,A,E8.2,A)') "f-Total (Laje)= ", ft
		WRITE (6, '(1X,A,E8.2,A)') "LP= ", LP


    
	WRITE (6, '(/,/,1X, A,I3,A)') "--- DADOS DA VIGA INDIVÍDUO ", k, " ---"


		WRITE (6, '(1X,A,I3,A)') "VV= ", VV
!		WRITE (6, '(1X,A,E8.3,A)') "XV= ", XV
!		WRITE (6, '(1X,A,E8.3,A)') "BXV= ", BXV
!		WRITE (6, '(1X,A,E8.3,A)') "IV= ", IV
!		WRITE (6, '(1X,A,E8.3,A)') "YGV= ", YGV
!		WRITE (6, '(1X,A,E8.3,A)') "IVC= ", IVC
!		WRITE (6, '(1X,A,E8.3,A)') "YGCV= ", YGCV
!		WRITE (6, '(1X,A,E8.3,A)') "AVC= ", AVC
!		WRITE (6, '(1X,A,E9.3,A)') "PAV= ", PAV
!		WRITE (6, '(1X,A,E9.3,A)') "PAT= ", PAT
!		WRITE (6, '(1X,A,E9.3,A)') "SOMA= ", SOMA
!		WRITE (6, '(1X,A,E9.3,A)') "SOMA2= ", SOMA2
!		WRITE (6, '(1X,A,E9.3,A)') "SOMA3= ", SOMA3
!		WRITE (6, '(1X,A,E9.3,A)') "SPTV= ", SPTV
!		WRITE (6, '(1X,A,E9.3,A)') "WVI= ", WVI
!		WRITE (6, '(1X,A,E9.3,A)') "VTINPT= ", VTINPT
!		WRITE (6, '(1X,A,E9.3,A)') "WVI= ", WVI

!		WRITE (6, '(1X,A,E8.3,A)') "MVD= ", mvd
!		WRITE (6, '(1X,A,E8.3,A)') "MresViga= ", mresv
		WRITE (6, '(1X,A,E8.2,A)') "cf-Total (Viga)= ", cftv
		WRITE (6, '(1X,A,E8.2,A)') "f-Total (Viga)= ", ftv

!		WRITE (6, '(1X,A,E8.2,A)') "LPV= ", LPV
!		WRITE (6, '(1X,A,E8.2,A)') "NVP= ", NVP
!		WRITE (6, '(1X,A,E8.2,A)') "QDPV= ", QDPV
!		WRITE (6, '(1X,A,E8.2,A)') "PTAV= ", PTAV
		WRITE (6, '(1X,A,E8.2,A)') "qdv= ", qdv

!		WRITE (6, '(1X,A,E8.2,A)') "LPL= ", LPL
!		WRITE (6, '(1X,A,E8.2,A)') "NLP= ", NLP
		WRITE (6, '(1X,A,I3,A)') "QDP= ", QDP
		WRITE (6, '(1X,A,E8.2,A)') "QDL= ", QDL
!		WRITE (6, '(1X,A,E8.2,A)') "PTAL= ", PTAL
...
!																   !
!																   !
!                     TESTE ACHAR as restrições violadas		   !
!																   !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

         	DO I=1,17
			IF (GL(I).NE.0) THEN
			                WRITE(6, '(/,1X,A,I2,A,E8.2)') "GL", I,"=",gl(i)
							ENDIF
			ENDDO
			DO I=1,20
			IF (GV(I).NE.0) THEN
			             WRITE(6, '(/,1x,A,I2,A,E8.2)') "GV", I,"=",gv(i)
						 ENDIF
			ENDDO		
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!                            FIM TESTE								 !
